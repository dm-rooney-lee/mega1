...
/**
 * Level 3 — "Traps": the state-machine hazards (group C), introduced one at a
 * time. Pop-up-spike rhythm → a thwomp corridor → an arrow gauntlet with cover.
 * No turrets here — projectile enemies get their own level (4) so the ramp from
 * level 2 stays gentle. Coordinates are world pixels; expect to playtest-tune.
 */
