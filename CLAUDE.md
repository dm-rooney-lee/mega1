...
- 새 화면(씬)을 만들 때는 `create()`에서 `playBgm(this)`나 `stopBgm()` 중 하나를 반드시 호출한다. 배경음악은 씬 생명주기 밖(오디오 모듈)에 있어서, 둘 다 호출하지 않는 화면은 이전 화면이 남긴 음악 상태를 그대로 물려받는다 — 예를 들어 일시정지 화면을 추가하면 플레이 화면이 꺼둔 음악이 무음으로 남는다.
