# "Let's go" 프로젝트 AI 활용 보고서

## 1. 개요

우리팀은 2D 플랫포머 게임 개발 프로젝트 `Let's go`(웹 게임 제작에 쓰이는 도구 Phaser 4와 프로그래밍 언어 TypeScript로 만든 플랫포머 게임)을 진행하면서 AI를 활용했습니다. 이 보고서는 그 과정을 정리한 것입니다.

- **작업 기간**: 2026-07-21 ~ 2026-08-08 (18일). 이 중 커밋이 있었던 날은 10일이었습니다.
- **다루는 범위**: 어떤 AI 도구를 썼는지, 요청(프롬프트)을 어떤 방식으로 작성했는지, 실제로 어떤 작업에 AI를 활용했는지 세 가지입니다.
- **근거**: 
  - 아래 내용은 모두 이 저장소(코드와 문서가 버전별로 저장되는 공간, 여기서는 git 저장소)에 실제로 남아있는 커밋(작업 내용을 하나의 단위로 저장한 기록) 이력과 문서를 근거로 작성했습니다. 
  - 각 커밋을 식별하는 고유 코드(커밋 해시)를 함께 표기해, 필요하면 직접 확인할 수 있게 했습니다. 
  - 우리팀은 간단한 수정 작업까지 포함해 개발 전체에 AI를 사용했습니다 — 커밋 메시지에 AI를 공동작업자로 명시적으로 남긴 것은 그중 일부이며, 자세한 내용은 2장에서 설명합니다.

## 2. 사용한 AI 도구

**Claude Code** 
- 시장 리서치, 게임 콘텐츠 개발 및 디자인, 게임 플레이 요소 개발 등 기획부터 플레이 가능한 게임을 개발하는 전 과정에서 Claude Code를 활용하였습니다. 
- Harness 로는 Claude Code의 공식 플러그 인인 superpowers를 사용하였습니다. 
- 프롬프트를 제대로 이해하고 높은 퀄리티의 결과를 만들어 내기 위해 in context learning 과정을 거쳤습니다.
...
| Claude Opus 4.7 (초대형 문맥) | 4건 |
| Claude (버전 미표기) | 4건 |
| Claude Opus 4.8 | 2건 |
| Claude Opus 5 | 1건 |

- 개발 초기에는 일반적으로 더 좋은 모델로 알려진 Opus 5를 통해 개발을 진행했습니다. 
- 하지만 속도가 느리고 토큰을 많이 먹는 문제가 있어서 좀 더 가벼운 모델인 Sonnet 5로 개발을 진행했습니다. 
- Sonnet 5를 사용하면서 Opus 5 대비 실제 추론이나 구현 퀄리티에서 차이를 느끼지 못했고 개발 속도나 퀄리티 면에서 Sonnet 5로 충분함을 느꼈습니다. 
...
`CLAUDE.md`는 폴더마다 따로 두지 않고 저장소 루트에 하나만 두었습니다. 폴더마다 별도의 지침 파일을 두는 방식은 코드 규모가 큰 저장소에서 한 파일에 너무 많은 내용이 몰리는 것을 막기 위한 것인데, 이 프로젝트는 파일 수가 많지 않아 그렇게까지 나눌 필요가 없다고 판단했습니다. 그 대신 여러 폴더에 걸쳐 반복되는 관례만 `.claude/rules/`에 경로별로 스코프를 지정해 분리했습니다.
...
이 사례에서는 구현 내용이 설계 문서가 정한 방향(각 신규 몬스터를 기존 어떤 클래스를 참고해서 만들지 등)을 그대로 따랐고, 구현 과정에서 생긴 사소한 버그 몇 건만 나중에 고쳤습니다.

이 프로젝트에는 기능마다 날짜가 붙은 설계 문서 9개와 작업 목록 문서 6개가 남아 있어, 이것이 예외적인 경우가 아니라 프로젝트 전체의 기본 작업 방식이었음을 보여줍니다.

### 3.4 반복 작업을 절차 문서로 표준화

새로운 장애물이나 몬스터를 하나 추가하려면 7~8개 파일을 순서에 맞춰 고쳐야 하는데, 이런 반복 작업은 절차 문서(`.claude/skills/add-hazard-type/SKILL.md`)로 만들어 표준화했습니다. 이후에는 이 절차 문서를 참고하면 빠뜨리는 파일 없이 작업할 수 있습니다.
...
두 경우 모두 같은 패턴입니다: 우리팀은 Gemini로 만든 초안을 실제 게임의 제약(화면 비율, 그리는 방식, 필요 없는 요소 제거)에 맞게 Claude Code로 다시 그려서 반영했습니다.
