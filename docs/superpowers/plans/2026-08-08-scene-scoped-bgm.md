...
> ⚠️ **이 계획은 실행이 끝났고, 결과는 그 뒤 한 번 더 바뀌었다.** 플레이 중 완전 무음이 허전해서 "음악을 끈다"를 "음악 볼륨을 낮춘다"로 바꿨다. 그 과정에서 이 문서가 만들라고 한 `stopBgm()`은 삭제됐고, `playBgm`은 볼륨 인자를 받게 됐으며, 여기서 "범위 밖"이라던 볼륨 값은 `src/config.ts`로 옮겨졌다. **현재 동작은 `docs/superpowers/specs/2026-08-08-bgm-scene-scoped-design.md`의 13절**에 있다. 이 문서의 코드를 그대로 옮겨 쓰지 말 것.
...
**개발용 주소 파라미터는 일부 확인에만 쓸 수 있다.** `?stage=8`을 붙이면 마지막 스테이지부터 시작해 7번과 9번을 빠르게 확인할 수 있고, 6번(스테이지 1 → 2)은 `?stage=1`로 시작해 첫 스테이지를 깨면 된다.

더 오래된 개발용 파라미터인 `?level=N`(`http://localhost:5173/?level=8`)도 있다. 이걸 쓰면 마지막 스테이지부터 시작하면서도, `?stage=8`과 달리 그 뒤 타이틀로 돌아갈 수 있어 7·8·9번을 한 번에 확인할 수 있다. 이유는 두 파라미터가 읽히는 시점이 다르기 때문이다 — `?stage=N`은 타이틀 화면(`MenuScene.create()`)이 열릴 때마다 다시 확인되어 매번 곧장 플레이 화면으로 돌려보내지만, `?level=N`은 게임을 처음 켤 때(`BootScene.create()`) 딱 한 번만 읽힌다. 그래서 이후 타이틀로 돌아오면 개발용 분기에 다시 걸리지 않고 타이틀 화면이 정상적으로 뜬다.

단 **1·4·8번은 반드시 파라미터 없는 주소**(`http://localhost:5173`)로 확인한다. 주소에 `?stage=N`이 남아 있으면 타이틀 화면이 뜰 때마다 개발용 분기가 곧장 플레이 화면으로 다시 보내버려서, 타이틀에 머무는 1번과 `ESC`로 타이틀에 돌아가는 4·8번을 확인할 수 없다. 이번 변경과 무관한 원래 동작이다.
