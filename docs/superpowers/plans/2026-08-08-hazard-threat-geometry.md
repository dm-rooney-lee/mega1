...
- 새 발판 2개는 지면에 얹혀 있고(밑면 496), 넘어야 할 높이 44px는 점프 165px에 비해 부담이 없으며, 가시·깃발·이동 발판과 겹치지 않는다.
- 7스테이지 적(x=4260)의 순찰 구간 안에 받침대가 들어가지만, 적은 벽에 부딪히면 방향을 되돌리므로(`Enemy.ts:48`) 갇히지 않고 순찰 폭만 4204~4300으로 짧아진다.
