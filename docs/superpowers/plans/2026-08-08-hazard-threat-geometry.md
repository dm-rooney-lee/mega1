...
전수 검사는 처음 붙였을 때 **26건 실패**로 열렸고(깃발 8, 대포 12, 화살 4, 진자 2, 톱니 1 — 감사표와 일치), 교정 후 전부 통과했다. 최종 `npm test` 202개 통과, `npm run build` 통과.

계획과 달랐던 점:

- **Task 4와 5의 대포 관련 변경을 한 번에 처리했다.** 하늘 대포는 좌표 한 줄에 "표면 규약 전환"과 "발판 위를 훑도록 이동"이 함께 들어가, 나눠 쓰면 오히려 중간 상태가 이상해진다.
- **3스테이지 진자는 전수 검사를 이미 통과하고 있었다.** 걸어다니는 통로가 아니라 그 옆 이동 발판 탑승자를 4px 스치고 있었기 때문이다. 검사 기준으로는 위협이지만 설계 의도(통로를 지키는 것)와 다르므로 계획대로 매다는 지점을 내렸다. 지금은 통로와 이동 발판 둘 다를 확실히 위협한다.
- **위협 판정에 "몸 폭" 규칙을 추가했다.** 세로로만 판정하면 대포가 발판 끝을 10px 훑는 것도 위협으로 통과해, 정작 잡으려던 장식용 하늘 대포가 빠져나갔다. 주인공이 설 수 있으려면 몸 폭(24px)만큼의 표면이 필요하므로 그 조건을 넣었다.
- **읽기용 리포트 도구(`src/levels/audit.report.ts`)를 추가했다.** 통과/실패만으로는 "몇 px 모자라는지"를 알 수 없어 좌표 튜닝이 어렵다. 앱에서 import하지 않으므로 번들에는 들어가지 않는다(모듈 수·번들 해시 변화 없음으로 확인).

코드 리뷰가 잡아낸 것 (전부 수정 완료):

- **5스테이지 고정 터렛을 지면으로 내린 게 오히려 통과 불가를 만들었다.** x=600은 선반(520~680) 바로 아래라, 점프해도 천장에 막혀 몸통 바닥이 462까지만 올라가는데 발사선이 461~471이라 1px 차이로 못 피한다. 게다가 선반이 터렛을 완전히 덮어 "위에서 밟기"도 불가능했다. 선반 밖 x=760으로 옮겨 둘 다 해결했다.
- **대포 포탄이 대포 위에 선 주인공 몸속에서 생겨났다.** 포탄이 스프라이트 중심에서 나오는데 그 자리가 사람이 설 수 있는 발판 위라, 이동 거리 0에 예고 0인 죽음이 됐다. 화살 발사기처럼 포신 밖에서 나오도록 고쳤다(대포 6문 전부 해소 확인).
- **7스테이지 대포 받침대가 적 순찰을 반으로 잘랐다.** 받침대를 없애고 대포를 지면으로 되돌렸다. 이동 발판 탑승자 대신 지상 구간을 위협하지만, 건너는 행위 자체는 구덩이와 발판 타이밍이 이미 막고 있다.
- **7스테이지 진자를 내렸더니 호가 통로 밖 구덩이 점프까지 덮었다.** 두 진자를 각각 20px·40px 왼쪽으로 옮겨 호를 통로 안에 두었다. 점프 궤적과 겹치는 순간이 없음을 계산으로 확인했고, 사이 안전지대는 27px 남는다.
- **검사 규칙 자체의 오류 5건.** 몸 폭만큼 덮여야 위협으로 본 규칙은 Arcade 판정과 다르다(주인공이 발판 끝에 서면 몸이 12px 밖으로 나오므로 1px만 걸쳐도 죽는다) → 발판을 몸 절반만큼 넓혀 겹침만 보도록 고쳤다. 그 밖에 무너지는 발판·함정 바닥·스프링을 설 수 있는 곳으로 세던 것, 이동·소멸 발판을 설치 대상으로 인정하던 것, 발사체 사정거리(900px)를 무시하던 것, 팝업 가시의 가로 폭을 그림 기준으로 재던 것을 모두 고쳤다.
- **리포트 도구가 판정을 중복 구현하고 미설치 도구(tsx)를 요구했다.** 도구를 없애고, 검사 실패 메시지가 위험 구간 수치를 직접 찍도록 했다.

확인한 부작용 없음:

- 새로 세운 발판은 6스테이지 저지 기둥 하나뿐이다. 지면에 얹혀 있고(밑면 496), 넘어야 할 높이 44px는 점프 165px에 비해 부담이 없으며, 가시·깃발·적 순찰과 겹치지 않는다.
- 7스테이지 적(x=4260)의 순찰 구간은 받침대를 없애면서 4204~4400 전 구간으로 복구됐다.
